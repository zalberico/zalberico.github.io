...
Learning about data structures is the most important next step after getting a handle on a the syntax of a language and being able to write simple programs. There are a few core data structures that are pretty well detailed in the [Cracking the Coding Interview Book][CTCI] (along with example problems). Confusingly, languages tend to have different names for their implementation of the same data structures (Python calls hash tables 'dictionaries' for example), but most languages will have some implementation of the core data structures even if they have a unique name.
...
**[Less Wrong][Less Wrong]:** Not programming focused, but there's a decent amount of overlap between the rationality community and the programming community and I like a lot of the writing there, definitely something I would have liked to have found around the same time I found How to Become a Hacker. Here's an example post I like a lot: [Disputing Definitions][Disputing Definitions]
